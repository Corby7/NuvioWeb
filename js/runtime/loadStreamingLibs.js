// The bundled copies are the primary source: the app is packaged to run from
// file:// on TV, where a CDN round-trip is the slowest and least reliable part
// of starting an HLS/DASH stream. The CDN stays as a fallback for the browser
// build and for any packaging path that skipped the copy step in build.mjs.
...
function loadScript(src) {
  return new Promise((resolve, reject) => {
    ...
    script.onerror = (error) => {
      // Drop the failed tag so a retry from another source starts clean.
      script.remove();
      reject(error);
    };
