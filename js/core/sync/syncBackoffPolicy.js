// Cooldown shared by every sync cycle. Without it a backend outage (api.nuvio.tv
// has answered 522 for whole sessions) means StartupSyncService burns its three
// pull attempts every SYNC_INTERVAL_MS for as long as the TV is on, and each
// failed cycle is a Luna-proxied round trip on webOS.
//
// Only transient failures arm the cooldown: a 4xx or a missing RPC is a real
// answer from a healthy server and retrying it slower helps nobody. The state is
// persisted so a reload during an outage does not reset the ladder.
