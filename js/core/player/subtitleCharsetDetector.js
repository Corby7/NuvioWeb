// Charset detection for external subtitle payloads.
//
// Addons serve plenty of non-UTF-8 SRT/ASS (SubDL and OpenSubtitles especially),
// and the declared subtitle language is often missing or wrong. Order of trust:
// a BOM, then a valid-UTF-8 probe, then an explicit Content-Type charset, then
// the language hint, then byte-frequency detection. The last step is what covers
// CJK/Thai/Hebrew/Arabic, which a language-map alone cannot.
//
// repairDoubleEncodedUtf8IfNeeded handles the separate, very common case where
// the bytes are already-decoded mojibake rather than a legacy codepage.
