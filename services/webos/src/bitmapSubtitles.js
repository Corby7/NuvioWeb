...
var CODEC_VOBSUB = "S_VOBSUB";
var CODEC_PGS = "S_HDMV/PGS";

function bitmapFormatForCodecId(codecId) {
  if (codecId === CODEC_VOBSUB) return "vobsub";
  if (codecId === CODEC_PGS) return "pgs";
  return "";
}
...
// A Matroska S_HDMV/PGS block holds one or more bare PGS segments: a 1-byte
// type, a 2-byte big-endian length, then the payload. The .sup container that
// decoders expect additionally prefixes each segment with "PG" plus 32-bit PTS
// and DTS, because in Matroska the timing lives on the block instead.
function validatePgsPayload(payload) {
  if (!payload || payload.length < 3) return false;
  var offset = 0;
  while (offset + 3 <= payload.length) {
    var segmentLength = payload.readUInt16BE(offset + 1);
    offset += 3 + segmentLength;
    if (offset > payload.length) return false;
  }
  return offset === payload.length;
}

function appendPgsSupRecords(chunks, timestampMs, payload) {
  // PTS is a 90 kHz clock, wrapped to 32 bits like a real .sup stream.
  var pts = Math.max(0, Math.round(timestampMs * 90)) >>> 0;
  var offset = 0;
  var written = 0;
  while (offset + 3 <= payload.length) {
    var segmentLength = payload.readUInt16BE(offset + 1);
    var segmentEnd = offset + 3 + segmentLength;
    if (segmentEnd > payload.length) break;
    var header = Buffer.alloc(10);
    header[0] = 0x50; // 'P'
    header[1] = 0x47; // 'G'
    header.writeUInt32BE(pts, 2);
    header.writeUInt32BE(0, 6); // DTS is unused by the decoders we target.
    chunks.push(header);
    chunks.push(payload.slice(offset, segmentEnd));
    written += header.length + (segmentEnd - offset);
    offset = segmentEnd;
  }
  return written;
}
...
function parseBlock(data, element, track, clusterTicks, timecodeScaleNs) {
  ...
  if ((flags & 0x06) !== 0) {
    throw bitmapSubtitleError("LACED_BITMAP_SUBTITLE", "Laced Matroska bitmap subtitle blocks are not supported");
  }
  ...
  var blockFormat = bitmapFormatForCodecId(track.codecId);
  if (blockFormat === "pgs") {
...
async function buildWindow(mediaUrl, trackNumber, startSeconds, endSeconds) {
  ...
  if (trackFormat === "pgs") {
    // PGS needs no side-channel index: the reconstructed .sup stream is
    // self-describing, so only the byte payload is returned.
    uniqueFrames.forEach(function (frame) {
      outputLength += appendPgsSupRecords(chunks, frame.timestampMs, frame.payload);
      if (outputLength > MAX_WINDOW_BYTES) {
        throw bitmapSubtitleError("WINDOW_TOO_LARGE", "PGS window exceeded its safety limit");
      }
    });
    var supData = Buffer.concat(chunks);
    return {
      format: "pgs",
      ...
      idxContent: "",
      subBase64: supData.toString("base64"),
      subBytes: supData.length
    };
  }
...
async function prepareBitmapSubtitleSource(options) {
  ...
  var bitmapTracks = metadata.tracks.filter(function (track) {
    return track.type === 0x11 && bitmapFormatForCodecId(track.codecId);
  });
  return {
    prepared: true,
